...
function initPagination() {
    ...
    if (statusFilter) {
        statusFilter.addEventListener('change', () => loadPaymentsWithPagination(1));
    }
    
    const tenantFilter = document.getElementById('tenantFilter');
    if (tenantFilter) {
        tenantFilter.addEventListener('change', () => loadPaymentsWithPagination(1));
    }
    
    const imageTenantFilter = document.getElementById('imageTenantFilter');
    if (imageTenantFilter) {
        imageTenantFilter.addEventListener('change', () => loadImagesWithPagination(1));
    }
}
...
async function loadPaymentsWithPagination(page = 1) {
    ...
    try {
        ...
        console.log('Payment filters:', { statusFilter, tenantFilter, searchInput });
    ...
    payments.forEach(payment => {
        ...
        html += `
            <tr>
                ...
                <td>${payment.previous_meter?.toLocaleString() || '0'}</td>
                <td>${payment.current_meter?.toLocaleString() || '0'}</td>
                <td>${electricityUsage?.toLocaleString() || '0'}</td>
